...
void drawSpiffsJpeg(const char *filename, int xpos, int ypos) {
...
void setup() {
  ...
  tft.begin();
  tft.setRotation(MMB_ROTATION);  // portrait
  tft.fillScreen(random(0xFFFF)); //random background color
}
...
void loop() {
  if(up.clicked()){
    drawSpiffsJpeg("/pacman.jpg", 0, 0);
  }
  if(down.clicked()){
    drawSpiffsJpeg("/1943.jpg", 0, 0);
  }
}
